...
export const ApplicationForm: React.FC<ApplicationFormProps> = ({
  ...
  const onFormSubmit = async (data: ApplicationFormData) => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to save job application";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fade-in">
      <div className="bg-slate-900 border border-slate-800 w-full max-w-xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Modal Header */}
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="p-6 space-y-4 overflow-y-auto">
          {errors.root && (
            <div className="bg-rose-500/10 border border-rose-500/30 text-rose-400 p-3 rounded-md text-sm">
              {errors.root.message}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                } rounded-md px-3.5 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500`}
              />
              {errors.company && (
                <p className="text-xs text-rose-400 mt-1">{errors.company.message}</p>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                {...register("status")}
                className="w-full bg-slate-800 border border-slate-700 rounded-md px-3.5 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full bg-slate-800 border ${
                  errors.appliedDate ? "border-rose-500" : "border-slate-700"
                } rounded-md px-3.5 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500`}
              />
              {errors.appliedDate && (
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 rounded-md px-3.5 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
          ...
          <div className="pt-4 flex items-center justify-end gap-3 border-t border-slate-800">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold rounded-md text-sm transition cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              className="px-5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-semibold rounded-md text-sm shadow-lg shadow-blue-500/25 transition cursor-pointer disabled:opacity-50"
            >
